...
/** --- 压缩 --- */
function terserPlugin(es, protect = false) {
    return {
        'name': 'pre-terser',
 ...
 * @param path 当前包内目录
 */
async function collectAppFiles(files, base, path = '') {
 ...
 * @param icon 应用图标
 */
async function buildCga(files, config, icon) {
    ...
    const manifestData = encryptCga(Buffer.from(JSON.stringify(manifest)), key, manifestNonce, getCgaAad(packageId, 'manifest'));
 ...
 * @param config 应用配置
 */
function createCgaGroups(files, config) {
    ...
    const groups = startup.length ? [startup] : [];
    let group = [];
    let size = 0;
 ...
 */
function maskCgaSeed(seed, salt, packageId) {
 ...
 * @param packageId 包 ID
 */
function deriveCgaKey(seed, salt, packageId) {
 ...
 * @param aad 附加认证数据
 */
function encryptCga(data, key, nonce, aad) {
