...
/** --- 当前启用系统文件打开处理的启动类 --- */
let fileOpenBoot: AbstractBoot | undefined;
 ...
 * @returns 无
 */
function activateMainForm(): void {
 ...
 */
function normalizeFilePaths(paths: string[], cwd: string): string[] {
    const result: string[] = [];
 ...
 * @returns 无
 */
function dispatchOpenFiles(paths: string[], cwd: string): void {
    if (!fileOpenBoot) {
        return;
    }
...
export abstract class AbstractBoot {
     ...
     * @returns 页面是否有对应监听器
     */
    public async emit(name: string, ...param: any[]): Promise<boolean> {
        if (!form || !token || mainNavigating || !name) {
            return false;
        }
        try {
            return (await form.webContents.executeJavaScript(
                `Boolean(window.clickgoNativeWeb?.invoke(${JSON.stringify(name)}, ...${JSON.stringify(param)}))`
            )) === true;
        }
        catch {
            return false;
...
export interface ILauncherOptions {
    /** --- 是否处理文件关联、命令行和拖入窗体的文件，默认 false --- */
    'openFiles'?: boolean;
    /** --- 是否限制为单一应用实例，默认 false --- */
 ...
 * @returns 无
 */
function resetMainSession(): void {
    token = '';
    methods['cg-init'] = {
        'once': true,
 ...
 * @returns 无
 */
export function launcher(boot: AbstractBoot, options: ILauncherOptions = {}): void {
    (async function() {
        ...
        if (options.openFiles) {
            electron.app.on('open-file', function(event, path): void {
        ...
        if (options.singleInstance) {
            electron.app.on('second-instance', function(_event, argv, cwd): void {
                activateMainForm();
                if (options.openFiles) {
                    const offset = process.defaultApp ? 2 : 1;
...
electron.ipcMain.handle('pre', function(e: electron.IpcMainInvokeEvent, name: string, ...param: any[]): any {
    if (
        !form || form.isDestroyed() || mainNavigating ||
...
electron.ipcMain.on('drop-files', function(event, paths: unknown): void {
...
electron.ipcMain.on('file-open-enabled', function(event): void {
 ...
 */
function getPageUrl(value: string): string {
    try {
...
function createForm(p: string, opt: {
    ...
    form.webContents.on('will-navigate', (event): void => {
        if (getPageUrl(event.url) !== mainPage) {
            event.preventDefault();
        }
    });
    form.webContents.on('will-redirect', (event): void => {
    ...
    form.webContents.on('did-start-navigation', (event): void => {
        if (event.isMainFrame && !event.isSameDocument && (getPageUrl(event.url) === mainPage)) {
            mainNavigating = true;
            resetMainSession();
        }
    });
    form.webContents.on('did-navigate', (): void => {
        mainNavigating = false;
    ...
    form.webContents.on('did-fail-load', (_event, _code, _description, _url, isMainFrame): void => {
        if (isMainFrame) {
            mainNavigating = false;
        }
    });
    form.webContents.on('render-process-gone', (): void => {
        token = '';
        mainNavigating = true;
    });
