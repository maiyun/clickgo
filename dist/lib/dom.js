 ...
 * @param el 要监听的元素
 */
function createWatchSizeInfo(el) {
 ...
 * @param immediate 立刻先执行一次回调
 */
export function watchSizeMulti(current, el, cb, immediate = false) {
    if (typeof current !== 'string') {
        current = current.taskId;
    }
    if ((current !== sysId) && !lTask.getOrigin(current)) {
 ...
 */
export function unwatchSizeMulti(current, el, cb) {
    if (typeof current !== 'string') {
        current = current.taskId;
    }
    const info = getWatchSizeInfo(el);
