...
function checkCgaLoadEnvironment(current: string): boolean {
    if (
        clickgo.isNative() ||
...
export async function run(
    ...
    if (app.config.style) {
        ...
        if (style) {
            ...
            lDom.pushStyle(taskId, await lTool.styleUrl2DataUrl(
                app.config.style, r.style, (path) => app.package.getContent(path)
            ));
        }
    }
    // --- 触发 taskStarted 事件 ---
...
export interface ITaskRunOptions {
    ...
    /** --- 返回总加载进度百分比（0 - 1） --- */
    'perProgress'?: (per: number) => void | Promise<void>;
    /** --- 显示 notify 窗口 --- */
    'notify'?: boolean;
