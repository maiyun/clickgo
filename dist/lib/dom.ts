...
/** --- RTL 横向原生 scrollLeft 的实现类型 --- */
type TRtlScrollType = 'default' | 'negative' | 'reverse';

/** --- 浏览器 RTL 横向原生 scrollLeft 的实现类型 --- */
let rtlScrollType: TRtlScrollType | undefined;

/**
 * --- 获取当前浏览器 RTL 横向原生 scrollLeft 的实现类型 ---
 * @returns 原生 scrollLeft 的实现类型
 */
function getRtlScrollType(): TRtlScrollType {
    if (rtlScrollType) {
 ...
 * @returns 逻辑横向滚动位置
 */
export function getScrollInlineOffset(el: HTMLElement): number {
    const max = Math.max(0, el.scrollWidth - el.clientWidth);
 ...
 */
export function setScrollInlineOffset(el: HTMLElement, value: number): void {
    const max = Math.max(0, el.scrollWidth - el.clientWidth);
    value = Math.min(max, Math.max(0, value));
...
const formStyleCache = new Map<string, Map<string, {
    'element': HTMLStyleElement;
    'refs': number;
}>>();

/** --- form/panel 实例持有的样式，task -> form -> panel --- */
const formStyleOwners = new Map<string, Map<string, Map<string, Set<string>>>>();
 ...
 * @returns 是否找到共享样式记录
 */
function removeFormStyle(taskId: string, formId: string, panelId?: string): boolean {
