...
/** --- 已完成隔离处理的 form/panel 视图源码 --- */
interface IPreparedView {
    'layout': string;
    'style': string;
    'prep': string;
}
...
const preparedViewCache = new WeakMap<lTask.ITask, Map<string, Map<string, Map<string, Promise<IPreparedView>>>>>();
 ...
 */
async function prepareView(task: lTask.ITask, path: string, layout: string, style: string): Promise<IPreparedView> {
    ...
    const preparing = (async (): Promise<IPreparedView> => {
        let prep = '';
        if (style) {
 ...
 */
function setLocaleAttributes(el: HTMLElement, locale: string): void {
...
export function refreshLocaleDirection(taskId?: string): void {
    ...
    const wraps = [
        ...
        ...elements.popList.querySelectorAll<HTMLElement>(selector),
    ];
...
interface IPopViewportStyle {
    'maxWidthSource': string;
    'maxWidthApplied': string;
    'maxHeightSource': string;
    'maxHeightApplied': string;
}
...
export function dialog(current: lCore.TCurrent, opt: string | IFormDialogOptions): Promise<string> {
    return new Promise(function(resolve) {
        ...
        const cls = class extends AbstractForm {
            ...
            public dialogWidth = nopt.width;

            public dialogHeight = nopt.height;

            public dialogPadding = nopt.padding;
