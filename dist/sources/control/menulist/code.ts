...
export default class extends clickgo.control.AbstractControl {
     ...
     * @returns void
     */
    public refreshScroll(scrolled: boolean = false): void {
        const body = this.refs.body;
        const max = body.scrollHeight - body.clientHeight;
        this.scrollStart = max > 1 && body.scrollTop > 1;
        this.scrollEnd = max > 1 && body.scrollTop < max - 1;
        if (scrolled) {
            const child = body.querySelector<HTMLElement>('[data-cg-pop-open]');
            if (child) {
     ...
     * @returns void
     */
    public scrollDown(event: PointerEvent, direction: 'start' | 'end'): void {
        const body = this.refs.body;
