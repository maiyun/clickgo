...
export default class extends clickgo.control.AbstractControl {
    ...
    public props: {
        ...
        /** --- 限定可选的最日期，如 20200101（含） --- */
        'start'?: string;
        /** --- 限定可选的最大日期，如 20250101（含） --- */
        'end'?: string;
    ...
    public get valueSet(): Set<string> {
        return new Set(this.values);
    }

    /** --- 禁用日期索引，避免每个日期格重复线性查找 --- */
    public get disabledSet(): Set<string> {
    ...
    /** --- 当前月份所有天数 --- */
    public monthMaps: string[] = [];

    /** --- 周对应的数组 --- */
    public weekMaps: string[][] = [];
    ...
    public checkClick(e: MouseEvent, col: {
        'year': string;
        'month': string;
        'date': string;
        'day': number;
