...
export default class extends clickgo.control.AbstractControl {
    ...
    public access: {
        ...
        'multi': boolean;
        'fitted': boolean;
        'last': clickgo.control.IPanzoomEditEvent | null;
    } = {
            'stop': null, 'editing': false, 'multi': false, 'fitted': false, 'last': null
        };

    public get contentWidthComp(): number {
        const value = this.propNumber('contentWidth');
        return Number.isFinite(value) ? Math.max(0, value) : 0;
    }

    public get contentHeightComp(): number {
     ...
     * @param y 视口内垂直坐标
     */
    public zoomTo(zoom: number, x = this.width / 2, y = this.height / 2): void {
        this.access.stop?.();
    ...
    public zoomFit(): void {
        ...
        this.setView(zoom,
            (this.width - this.contentWidthComp * zoom) / 2, (this.height - this.contentHeightComp * zoom) / 2);
    }
     ...
     */
    public toLocal(clientX: number, clientY: number): clickgo.control.IPanzoomPoint {
     ...
     */
    public gesture(event: PointerEvent | WheelEvent): void {
        if (this.propBoolean('disabled') || ((event.type === 'pointerdown') && ((event as PointerEvent).button !== 0))) {
            return;
        }
        ...
        const stop = clickgo.modules.pointer.scale(event, (e, factor, _cpos, detail) => {
            if (this.access.editing && (detail.mode === 'pan')) {
                this.access.last = { ...this.toLocal((e as PointerEvent).clientX, (e as PointerEvent).clientY), 'event': e };
            ...
            this.setView(zoom,
                detail.center.x - left - (detail.previousCenter.x - left - this.xData) * applied,
                detail.center.y - top - (detail.previousCenter.y - top - this.yData) * applied);
        }, {
            'target': this.element,
            'onPointers': (e, count) => {
                if (count > 1) {
    ...
    public keydown(event: KeyboardEvent): void {
        ...
        switch (event.key) {
            case '+':
            case '=': this.zoomTo(this.zoomData * 1.2); break;
            case '-': this.zoomTo(this.zoomData / 1.2); break;
    ...
    public onMounted(): void {
        ...
        clickgo.dom.watchSize(this, this.element, () => {
            ...
            if (this.access.fitted) {
                ...
                this.access.stop?.();
                this.setView(this.zoomData,
                    this.xData + (this.width - oldWidth) / 2, this.yData + (this.height - oldHeight) / 2);
            }
        }, true);
        ...
        this.watch(() => [this.props.modelValue, this.props.x, this.props.y], (next, old) => {
            ...
            this.setView(zoomChanged ? zoom : this.zoomData,
                xChanged ? this.propNumber('x') : this.width / 2 - (this.width / 2 - this.xData) * applied,
                yChanged ? this.propNumber('y') : this.height / 2 - (this.height / 2 - this.yData) * applied);
        });
        for (const name of ['contentWidth', 'contentHeight', 'autoFit'] as const) {
            this.watch(name, () => {
        ...
        for (const name of ['zoomMin', 'zoomMax'] as const) {
            this.watch(name, () => {
                if (this.access.fitted) {
                    this.zoomFit();
                }
        ...
        for (const name of ['disabled', 'mode'] as const) {
