 ...
 * @param names 累加结果
 */
function collectItemNames(item: ILayerItem, names: string[] = []): string[] {
    names.push(item.name);
    if (item.type === 'folder' && item.children) {
        for (const child of item.children) {
            collectItemNames(child, names);
        }
    }
    return names;
}
...
function isValidName(name: string): boolean {
    return name.length > 0 && name !== ARTBOARD_NAME;
}
...
export class LayerModule implements ILayerModule {
    ...
    public watch(name: keyof mHost.IFabricControl['props'], cb: () => void): void {
        const watch = this._host.watch as (name: keyof mHost.IFabricControl['props'], cb: () => void) => () => void;
        watch.call(this._host, name, cb);
    }

    public emit(name: string, ...v: unknown[]): void {
        const emit = this._host.emit as (name: string, ...v: unknown[]) => void;
        emit.call(this._host, name, ...v);
    }

    public get layerList(): ILayerItem[] {
        return (this._host as mHost.TFabricHost & { 'layerList': ILayerItem[]; }).layerList;
    }

    public set layerList(value: ILayerItem[]) {
        (this._host as mHost.TFabricHost & { 'layerList': ILayerItem[]; }).layerList = value;
    }
    ...
    /** --- 当前是否已应用拖拽中的简化控制点样式 --- */
    private _isDraggingStyle: boolean = false;

    /** --- 是否已安排在当前同步批次结束后统一刷新图层状态 --- */
    ...
    public layerUpdateStyle(isDragging: boolean): void {
        ...
        if (isDragging && this._isDraggingStyle) {
            return;
        }
        this._isDraggingStyle = isDragging;
        const showControls = !this.propBoolean('disabled') && this.propBoolean('transform') && !isDragging;
        ...
        if (!isDragging && activeObject instanceof this.access.fabric.ActiveSelection) {
            activeObject.set(props);
        }
    ...
    public layerApplyMode(): void {
        ...
        const disabled = this.propBoolean('disabled');
        this.access.canvas.forEachObject(obj => {
            if (isArtboard(obj)) {
                return;
            }
            ...
            // --- 禁用或特殊工具模式下所有对象均不可交互 ---
            if (disabled || this.props.mode === 'pan' || this.props.mode === 'zoom' || this.props.mode === 'marquee') {
        ...
        this.access.canvas.selection = !disabled && (this.props.mode !== 'marquee') && this.propBoolean('selector');
        this.layerUpdateStyle(false);
    }

    /**
     * --- 合并同一同步批次内的多次对象新增，只执行一次全画布状态刷新。 ---
     */
    public layerScheduleApplyMode(): void {
    ...
    public addLayer(name: string, title: string = name): boolean {
        if (!isValidName(name) || findItem(this.layerList, name)) {
            return false;
        }
        this.layerList.push({
            'type': 'layer',
    ...
    public removeLayer(name: string): void {
        ...
        const removedNames = collectItemNames(found.item);
        const removedNameSet = new Set(removedNames);
        const idx = found.parent.indexOf(found.item);
        found.parent.splice(idx, 1);
        // --- 删除该图层或文件夹所有子图层下的 fabric 对象 ---
        if (this.access.canvas) {
            const toRemove = this.access.canvas.getObjects().filter(obj => removedNameSet.has(getName(obj)));
            for (const obj of toRemove) {
                this.access.canvas.remove(obj);
            }
        }
        // --- 同步移除当前激活范围中的文件夹及其所有子图层 ---
        if (this.props.layer.some(n => removedNameSet.has(n))) {
            this.emit('update:layer', this.props.layer.filter(n => !removedNameSet.has(n)));
        }
        const event: clickgo.control.IFabricLayerlistchangeEvent = {
            'detail': { 'type': 'remove', 'names': removedNames }
        };
        this.emit('layerlistchange', event);
    }
    ...
    public moveLayer(names: string | string[], refName: string | null, position: 'before' | 'after' | 'inside'): boolean {
        ...
        if (nameList.length === 0 || new Set(nameList).size !== nameList.length ||
        ...
        /** --- 在修改树之前完整验证参考目标，保证失败时不改变原结构。 --- */
        const ref = (refName === null) ? null : findItem(this.layerList, refName);
        if (refName !== null && !ref) {
            return false;
        }
        if (position === 'inside' && ref?.item.type !== 'folder') {
            return false;
        }
        for (const { item } of founds) {
            if (item.type === 'folder' && item.children) {
                if (refName !== null && findItem(item.children, refName)) {
                    return false;
                }
                // --- 同时移动父文件夹与其子项会破坏树结构，拒绝该输入。 ---
                if (nameList.some(name => name !== item.name && findItem(item.children!, name))) {
                    return false;
                }
            }
        }
