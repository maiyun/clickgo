 ...
 */
export function extractOutlineEdges(rects, zoom, vpt) {
    if (rects.length === 0) {
        return [];
    }
 ...
 */
export function buildOutlinePolygons(edges) {
    if (edges.length === 0) {
        return [];
    }
    // --- 构建端点邻接表 ---
    ...
    for (let i = 0; i < edges.length; i++) {
        ...
        const polygon = [];
        let cur = { 'x': edges[i].x1, 'y': edges[i].y1 };
        let idx = i;
        while (!visited.has(idx)) {
            visited.add(idx);
            polygon.push({ 'x': cur.x, 'y': cur.y });
            const e = edges[idx];
            const next = (e.x1 === cur.x && e.y1 === cur.y)
                ? { 'x': e.x2, 'y': e.y2 }
                : { 'x': e.x1, 'y': e.y1 };
 ...
 * @returns 减去后的矩形列表
 */
export function subtractRect(existing, cut) {
    const result = [];
 ...
 * @returns 交集后的矩形列表
 */
export function intersectRect(existing, clip) {
    const result = [];
    for (const r of existing) {
...
export class MarqueeModule {
    ...
    _drawingRect = null;
    /** --- 是否正在拖拽创建新选区 --- */
    _isMarqueeDrawing = false;
    /** --- 选区绘制起始点（canvas 坐标）--- */
    _marqueeStartX = 0;
    _marqueeStartY = 0;
    /** --- 是否正在拖拽移动已有选区 --- */
    _isMarqueeMoving = false;
    /** --- 选区移动上一帧鼠标位置 --- */
    _marqueeMoveLastX = 0;
    _marqueeMoveLastY = 0;
    /** --- after:render 监听器引用 --- */
    _marqueeRenderHandler = null;
    // --- 内部辅助方法 ---
    /**
         * --- 将选区矩形限制在画板范围内 ---
         * @param rect 待裁剪矩形
    ...
    /**
         ...
         */
    _isPointInMarquee(screenX, screenY) {
    ...
    /**
         ...
         */
    marqueeResetDrag() {
        const hadDrawing = this._drawingRect !== null;
        ...
        if (this._isMarqueeMoving) {
            ...
            if (dx !== 0 || dy !== 0) {
                ...
                if (this._artboard) {
                    ...
                    let maxX = -Infinity;
                    let maxY = -Infinity;
                    for (const rect of this._marqueeRects) {
                        minX = Math.min(minX, rect.x);
                        minY = Math.min(minY, rect.y);
                        maxX = Math.max(maxX, rect.x + rect.width);
                        maxY = Math.max(maxY, rect.y + rect.height);
                    }
                    cdx = Math.max(this._artboard.left - minX, Math.min(cdx, this._artboard.left + this._artboard.width - maxX));
                    cdy = Math.max(this._artboard.top - minY, Math.min(cdy, this._artboard.top + this._artboard.height - maxY));
                }
    ...
    marqueeHandleMouseUp() {
        ...
        if (this._isMarqueeDrawing) {
            ...
            else if (this.props.marqueeCompose === 'replace') {
                this._marqueeRects = [];
            }
            this._drawingRect = null;
            this._marqueeSync();
    ...
    setMarqueeRect(x, y, width, height) {
        ...
        this._isMarqueeMoving = false;
        this._drawingRect = null;
        const rect = this._marqueeClipToArtboard({
            'x': Math.min(x, x + width),
            'y': Math.min(y, y + height),
            'width': Math.abs(width),
            'height': Math.abs(height),
        });
        this._marqueeRects = (rect && rect.width > 0 && rect.height > 0) ? [rect] : [];
        this._marqueeSync();
        this._marqueeClearContextTop();
        this.access.canvas?.renderAll();
    }
