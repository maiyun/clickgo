 ...
 * @param names 累加结果
 */
function collectItemNames(item, names = []) {
...
export class LayerModule {
    ...
    get propBoolean() {
        return this._host.propBoolean;
    }
    watch(name, cb) {
        const watch = this._host.watch;
        watch.call(this._host, name, cb);
    }
    emit(name, ...v) {
        const emit = this._host.emit;
        emit.call(this._host, name, ...v);
    }
    get layerList() {
        return this._host.layerList;
    }
    set layerList(value) {
        this._host.layerList = value;
    }
    /** --- layerApplyMode 正在以编程方式修改 canvas 选区，此期间压制 onSelectionChange 的反向 emit --- */
    _layerApplying = false;
    /** --- 当前是否已应用拖拽中的简化控制点样式 --- */
    _isDraggingStyle = false;
