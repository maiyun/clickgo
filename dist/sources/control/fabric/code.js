 ...
 */
const extractOutlineEdges = (rects, zoom, vpt) => {
    if (rects.length === 0) {
        return [];
    }
 ...
 */
const buildOutlinePolygons = (edges) => {
    if (edges.length === 0) {
        return [];
    }
...
export default class extends clickgo.control.AbstractControl {
    ...
    async onMounted() {
         ...
         */
        const isPointInMarquee = (screenX, screenY) => {
         ...
         */
        const mergeRects = (existing, newRect) => {
            return [...existing, newRect];
        };
        /**
         * --- subtract 模式：从已有矩形列表中减去新矩形重叠区域 ---
         */
        const subtractRect = (existing, cut) => {
            const result = [];
         ...
         */
        const intersectRect = (existing, clip) => {
            const result = [];
            for (const r of existing) {
         ...
         */
        const applyMarqueeCompose = (newRect) => {
        ...
        const setupAfterRender = () => {
            ...
            afterRenderHandler = () => {
                ...
                const stableRects = marqueeRects.filter(r => !r._temp);
                const tempRect = marqueeRects.find(r => r._temp);
                // --- 双色虚线绘制函数 ---
        ...
        this.access.canvas.on('mouse:move', (e) => {
            ...
            if (isMarqueeDrawing && this.access.canvas) {
                ...
                if (clipped) {
                    ...
                    marqueeRects = [...marqueeRects.filter(r => !r._temp), Object.assign(clipped, { '_temp': true })];
                }
                this.access.canvas.requestRenderAll();
                return;
            }
        ...
        this.access.canvas.on('mouse:up', () => {
            ...
            if (isMarqueeDrawing && this.access.canvas) {
                isMarqueeDrawing = false;
                // --- 找到临时矩形 ---
                const tempIdx = marqueeRects.findIndex(r => r._temp);
                if (tempIdx >= 0) {
                    const tempRect = { ...marqueeRects[tempIdx] };
                    delete tempRect._temp;
                    // --- 移除临时矩形，恢复原始列表 ---
                    const origRects = marqueeRects.filter(r => !r._temp);
