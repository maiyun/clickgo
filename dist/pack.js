...
for (const item of checkList) {
    ...
    if (emitsFullMatch) {
        // --- 从 emits 对象中提取所有的键 ---
        const emitsContent = emitsFullMatch[0];
        const emitsKeys = emitsContent.matchAll(/'([^']+)'\s*:/g);
        for (const match of emitsKeys) {
...
for (const item of list) {
    ...
    if (['bgroup', 'alert', 'alayout', 'alayout-row', 'alayout-cell', 'alayout2', 'alayout2-cell', 'grid', 'grid-cell', 'check', 'circle', 'content', 'dialog', 'dock', 'dock-group', 'dock-item', 'file', 'greatlist', 'greatselect', 'group', 'html', 'vflow', 'video', 'img', 'imgviewer', 'panzoom', 'label', 'layout', 'hske', 'icon', 'levelselect', 'list', 'loading', 'marquee', 'menu', 'menu-item', 'menulist', 'menulist-item', 'menulist-split', 'flow', 'eflow', 'radio', 'scroll', 'select', 'sgroup', 'slider', 'stab', 'stab-item', 'statusbar', 'statusbar-item', 'toolbar', 'toolbar-split', 'step', 'svg', 'switch', 'arrow', 'tab', 'tag', 'text', 'tip', 'title', 'task-item', 'table-item', 'nav-item', 'nav-title', 'panel', 'desc-body', 'desc-cell', 'desc-head', 'desc-header', 'desc-row', 'link', 'date', 'daterange', 'progress', 'datepanel', 'calendar', 'empty', 'palette', 'colorist', 'uploader', 'setting', 'setting-item', 'delete', 'timeline', 'timeline-item', 'number', 'web', 'objviewer-item'].includes(item.name)) {
        continue;
    }
    try {
        ...
        if (item.name === 'button') {
            ...
            paths.push(base + 'slider');
            paths.push(base + 'dock');
            paths.push(base + 'dock-group');
            paths.push(base + 'dock-item');
            paths.push(base + 'stab');
            paths.push(base + 'stab-item');
            paths.push(base + 'statusbar');
            paths.push(base + 'statusbar-item');
            paths.push(base + 'toolbar');
            paths.push(base + 'toolbar-split');
            ...
            paths.push(base + 'desc-body');
            paths.push(base + 'desc-cell');
            paths.push(base + 'desc-head');
            paths.push(base + 'desc-header');
